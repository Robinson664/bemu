//------------------------------------------------------------------------------
// <copyright project="BEmu_csh" file="IntradayBarRequest/IntradayBarElementResponseError.cs" company="Jordan Robinson">
...
namespace Bloomberglp.Blpapi.IntradayBarRequest
{
    ...
    internal class IntradayBarElementResponseError : Element
    {
        private readonly IntradayBarElementString _source, _category, _message, _subCategory;
        private readonly IntradayBarElementInt _code;

        internal IntradayBarElementResponseError(string security)
        {
            int code = Types.RandomDataGenerator.RandomInt(99);
            string sourceGibberish = Types.RandomDataGenerator.RandomString(5).ToLower();

            this._source = new IntradayBarElementString("source", string.Format("{0}::{1}{2}", code, sourceGibberish, Types.RandomDataGenerator.RandomInt(99)));
            this._code = new IntradayBarElementInt("code", code);
            this._category = new IntradayBarElementString("category", "BAD_SEC");
            this._message = new IntradayBarElementString("message", string.Format("Unknown/Invalid security [nid:{0}]", code));

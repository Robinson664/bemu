//------------------------------------------------------------------------------
// <copyright project="BEmu_csh" file="ReferenceDataRequest/ReferenceElementErrorInfo.cs" company="Jordan Robinson">
...
namespace Bloomberglp.Blpapi.ReferenceDataRequest
{
    ...
    internal class ReferenceElementErrorInfo : Element
    {
        private readonly ReferenceElementString _source, _category, _message, _subCategory;
        private readonly ReferenceElementInt _code;
        ...
        internal override StringBuilder PrettyPrint(int tabIndent, bool surroundValueWithQuotes = false)
        {
            ...
            result.AppendFormat("{0}errorInfo = {{{1}", tabs, Environment.NewLine);
            foreach (var item in this.Elements)
            {

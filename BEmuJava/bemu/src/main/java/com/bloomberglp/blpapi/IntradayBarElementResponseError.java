//------------------------------------------------------------------------------
// <copyright project="BEmu_maven" file="/BEmu_maven/bemu/src/main/java/com/bloomberglp/blpapi/IntradayBarElementResponseError.java" company="Jordan Robinson">
...
public class IntradayBarElementResponseError extends Element
{
    private final IntradayBarElementString _source, _category, _message, _subCategory;
    private final IntradayBarElementInt _code;

    IntradayBarElementResponseError(String security)
    {
        int code = RandomDataGenerator.randomInt(99);
        String sourceGibberish = RandomDataGenerator.randomString(5).toLowerCase();

        this._source = new IntradayBarElementString("source", String.format("%s::%s%s", code, sourceGibberish, RandomDataGenerator.randomInt(99)));
        this._code = new IntradayBarElementInt("code", code);
        this._category = new IntradayBarElementString("category", "BAD_SEC");
        this._message = new IntradayBarElementString("message", String.format("Unknown/Invalid security [nid:%s]", code));

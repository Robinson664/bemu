...
public class IntradayTickDataRequest
{
            ...
            while (continueToLoop)
            {
				...
				switch(evt.eventType().intValue())
				{
					case Event.EventType.Constants.RESPONSE:
						IntradayTickDataRequest.processResponseTick(evt, security);
		...
		while(miter.hasNext())
		{
			...
			for(int valueIndex = 0; valueIndex < elmTickDataArray.numValues(); valueIndex++)
			{
				...
				String out = String.format("%s: %s, %f @ %d",
						IntradayTickDataRequest.timeFmt.format(time.calendar().getTime()),
						type,
						value,
						size); 
